...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      data = await readData(); // re-read to get any changes
      const index = data.findIndex((item: any) => item.id === newId);

      if (index !== -1) {
        data[index].pro = parsedResult.正方观点;
        data[index].con = parsedResult.反方观点;
        data[index].pro_args = JSON.stringify(parsedResult.正方论点);
        data[index].con_args = JSON.stringify(parsedResult.反方论点);
      }
      ...
      data = await readData(); // re-read to get any changes
      const updateIndex = data.findIndex((item: any) => item.id === newId);

      if (updateIndex !== -1) {
        data[updateIndex].title = completedEssay.标题;
        data[updateIndex].content = completedEssay.正文;
      }
