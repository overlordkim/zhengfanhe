...
async function getTitleAndContent(args: { theme: string, shallowPositive: string, shallowNegative: string, deepPositive: string, deepNegative: string, positive:string, negative:string }) {
  ...
  //console.log(`${theme}\n以上是一篇高考作文题，我给你提供四个论点，分别是浅层肯定、浅层否定、深层肯定、深层否定，你使用“诚然、然而、当然、更进一步”串联起来四个层次，逻辑要流畅严密；并且在首尾额外补充一段破题段和一段总结段（所以，总共有六段，而且诚然段是第二段哦！）。你必须充分拓展观点的深意，做到深刻、有价值，而且必须涵盖我给你的观点的所有内容，不要遗漏。有八个要求：1、文章总共有六段：破题、浅层肯定、浅层否定、深层肯定、深层否定、总结（我只是告诉你这六段的总体内容，请不要在段落任何地方出现“浅层肯定,浅层否定,深层肯定,深层否定”这种提示词，并且注意不要把破题段和浅层肯定段叠在一起了，诚然段（浅层肯定段）是要单独成段，而且是第二段哦~）。2、中间四段中，每一段都要使用举例论证、引用论证，使论证更有力。要求：输出具体事例，输出相关的名人名言。具体事例一定要有确定的人名、时间、地点等，不要用“那些人、古代、历史上”等宽泛的指代去泛泛而谈。例子最好来源于我国古代的名人事迹，或是西方著名哲学家的思想名句，或是当今的社会热点、或是著名的历史事件；比如苏轼、欧阳修、白居易、鲁迅、屈原等文学大家的个人事迹、文学作品或者名言，或者红楼梦中的人物身世，又比如康德、柏拉图、亚里士多德、洛克等哲学家的思想，又比如俄乌冲突、种族歧视、环境保护、共克疫情等社会热点或者具体的某次历史事件。请你聚焦于人文社会、人类社会，充满人文关怀。举例之后，还得继续阐发举例/引用和本段论点的关系，体现思维的深度。3、对于两个深层段落，或是深刻理性，或是感人肺腑、抒情至深，而且都要有合适恰当的举例论证，或者引用名言论证。怎么抒情呢？你可以妙用反问句，比如“如果没有谁/什么，...？”，你可以适当学习，但不要直接照搬。4、文风优美理性，避免重复无意义词语，不要那些虚词，不要副标题5、标题控制在十个字以内，不要使用冒号，尽量使用最好的修辞（对偶，比喻，用典等）6、如何避免“深层肯定”和“浅层否定”之间的转折引起的前后矛盾？一种合理的方法是，在深层肯定的那一段，首先给论点添加一定的前置条件，这个前置条件可以来源于浅层否定中聚焦的那种弊端。7、深层否定是整篇文章最精华、最有深度的一段，如何避免它和前面冲突？一种合理的方法是从一个更高的视角看待整个问题，从更宏大、更深邃、普世价值、全人类、全社会等等的角度将整篇文章升华。8、整篇文章要内容翔实，多多举例、多多引用，请你努力填充丰富的内容和论证逻辑，体现缜密的思路。你必须非常注重段落之间的承接关系，不要出现“各打五十大板”的雨露均沾，你必须深刻认识到哪方才是更加正确的，必须要有判断正负的能力。返回一个dict，格式为{“标题”：标题，“正文”：正文}\n浅层肯定：${shallowPositive}浅层否定：${shallowNegative}深层肯定：${deepPositive}深层否定：${deepNegative}\n最后强调：中间四段每一段都必须需要输出具体的事例（历史事件、人物事迹、社会热点）/引用名言，并且要对具体事例做详实流畅的论述分析，不要只是说“这个事例体现了本段论点”；语言风格要优美地像诗一样，要有对诗词的巧妙化用，不是直接引用诗词，而是化用诗词的一部分，比如某个词语、某句诗，不要用引号引用。` );
    const response = await axios.post(url, payload);
    return response.data;
  }
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const titleAndContentArgs = {
        ...
        shallowNegative: `${parsedResult.反方论点[Object.keys(parsedResult.反方论点)[0]]}而且，${parsedResult.反方论点[Object.keys(parsedResult.反方论点)[1]]}`+extra2,
        ...
        deepNegative: `${parsedResult.反方论点[Object.keys(parsedResult.反方论点)[keys2.length - 1]]}而且，${parsedResult.反方论点[Object.keys(parsedResult.反方论点)[keys2.length - 2]]}同时，${parsedResult.反方论点[Object.keys(parsedResult.反方论点)[keys2.length - 3]]}`,
